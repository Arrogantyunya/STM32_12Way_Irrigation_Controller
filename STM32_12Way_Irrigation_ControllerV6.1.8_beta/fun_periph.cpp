...
void Some_Peripherals::Peripheral_GPIO_Config(void)
{
	...
	{
		Debug_Serial.println("No initialization configuration saved <Peripheral_GPIO_Config>");//未保存初始化配置
		iwdg_feed();
		...
		{
			Debug_Serial.println("ERROR!!! Set_Coil_DefaultValue Failure!!! <Peripheral_GPIO_Config>");//设置线圈初始值失败
			delay(1000);
	...
	{
		Debug_Serial.println("Setting of peripheral pin initialization <Peripheral_GPIO_Config>");//外围引脚初始化设置
		while (!Modbus_Coil.Set_Coil_InitValue())//设置线圈为初始值
		{
			Debug_Serial.println("ERROR!!! Set_Coil_InitValue Failure!!! <Peripheral_GPIO_Config>");//设置线圈初始值失败
			delay(1000);
			//这里不能喂狗，打印一定次数的错误报警信息后，看门狗自动复位单片机。
		}
	}
