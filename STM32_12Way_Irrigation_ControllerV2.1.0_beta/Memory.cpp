...
bool ModbusController_InitState::Save_DO_InitState(unsigned char *data)
{
	...
	if (DO_crc8 == DO_CRC8)
	{
		Serial.println("保存DO初始化值成功<Save_DO_InitState>");
		return true;
	}

	Serial.println("保存DO初始化值失败!!!<Save_DO_InitState>");
	return false;
}
...
{
	Serial.println("暂时未完成<Clean_DO_InitState>");
	return false;
}
...
bool ModbusController_InitState::Save_AO_InitState(unsigned char *data)
{
	...
	if (AO_crc8 == AO_CRC8)
	{
		Serial.println("保存AO初始化值成功<Save_AO_InitState>");
		return true;
	}

	Serial.println("保存AO初始化值失败!!!<Save_AO_InitState>");
	return false;
}
...
{
	Serial.println("暂时未完成<Read_AO_InitState>");
	return false;
}
...
{
	Serial.println("暂时未完成<Clean_AO_InitState>");
	return false;
}
...
bool ModbusController_InitState::Save_Timeout(unsigned char High, unsigned char Low)
{
	...
	{
		Serial.println("储存Timeout成功<Save_Timeout>");
		return true;
	}
	EEPROM_Write_Enable();
	...
	Serial.println("储存Timeout失败<Save_Timeout>");
	return false;
}
...
bool ModbusController_InitState::Clean_Timeout(void)
{
	...
	{
		Serial.println("清除Timeout成功<Clean_Timeout>");
		return true;
	}
	Serial.println("清除Timeout失败<Clean_Timeout>");
	return false;
}
