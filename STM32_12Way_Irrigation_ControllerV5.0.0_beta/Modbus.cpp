...
void Modbus::receivePDU(byte* frame) {
    byte fcode  = frame[0];
    word field1 = (word)frame[1] << 8 | (word)frame[2];//将起始地址高低字节并到一起
    word field2 = (word)frame[3] << 8 | (word)frame[4];//将数据量高低字节并到一起

	Debug_Serial.println(String("frame[0] = ") + String(frame[0], HEX));//0f
	Debug_Serial.println(String("field1 = ") + String(field1, HEX));//0000,起始地址
	Debug_Serial.println(String("field2 = ") + String(field2, HEX));//0040,数据量

    switch (fcode) {

        case MB_FC_WRITE_REG:
            //field1 = reg, field2 = value
            this->writeSingleRegister(field1, field2);//0x06写单个保持寄存器
        break;

        case MB_FC_READ_REGS:
            //field1 = startreg, field2 = numregs
            this->readRegisters(field1, field2);//0x03读保持寄存器
        break;

        case MB_FC_WRITE_REGS:
            //field1 = startreg, field2 = status
            this->writeMultipleRegisters(frame,field1, field2, frame[5]);//0x10写多个保持寄存器
        break;

        #ifndef USE_HOLDING_REGISTERS_ONLY
        case MB_FC_READ_COILS:
            //field1 = startreg, field2 = numregs
            this->readCoils(field1, field2);//0x01读线圈寄存器
        break;

        case MB_FC_READ_INPUT_STAT:
            //field1 = startreg, field2 = numregs
            this->readInputStatus(field1, field2);//0x02读离散输入寄存器
        break;

        case MB_FC_READ_INPUT_REGS:
            //field1 = startreg, field2 = numregs
            this->readInputRegisters(field1, field2);//0x04读输入寄存器
        break;

        case MB_FC_WRITE_COIL:
            //field1 = reg, field2 = status
            this->writeSingleCoil(field1, field2);//0x05写单个线圈寄存器
        break;

        case MB_FC_WRITE_COILS:
            //field1 = startreg, field2 = numoutputs
            this->writeMultipleCoils(frame,field1, field2, frame[5]);//0x0F写多个线圈寄存器
			//0F,0000,0040,08
        break;
...
void Modbus::exceptionResponse(byte fcode, byte excode) {
	Debug_Serial.println("异常响应!!!<exceptionResponse>");
    //Clean frame buffer
    free(_frame);
    _len = 2;
...
void Modbus::readInputStatus(word startreg, word numregs) {
	...
	Debug_Serial.println("Modbus::readInputStatus <readInputStatus>");
...
void Modbus::writeSingleCoil(word reg, word status) {
	...
	Debug_Serial.println(String("_reply = MB_REPLY_ECHO ") + "<writeSingleCoil>");
...
void Modbus::writeMultipleCoils(byte* frame,word startreg, word numoutputs, byte bytecount) {
	...
	Debug_Serial.println(String("bytecount_calc = ") + bytecount_calc);//64/8 = 8

    if (numoutputs%8) bytecount_calc++;
    if (numoutputs < 0x0001 || numoutputs > 0x07B0 || bytecount != bytecount_calc) {
		Debug_Serial.println("不应该进来的地方");
	...
	{
		Debug_Serial.println(String("frame[") + (i + 6) + "]= " + String(frame[i + 6], HEX));
	}
	Debug_Serial.println(">>>");
    ...
    // Begin_AUTOReceipt = true;//开启自动的上报
    _reply = MB_REPLY_NORMAL;
}
#endif
