...
void date::Update_RTC(unsigned char *buffer)
{
	...
	Debug_Serial.print("year: "); Debug_Serial.println(RTCTime.year);
	Debug_Serial.print("month: "); Debug_Serial.println(RTCTime.month);
	Debug_Serial.print("day: "); Debug_Serial.println(RTCTime.day);
	Debug_Serial.print("hour: "); Debug_Serial.println(RTCTime.hour);
	Debug_Serial.print("minutes: "); Debug_Serial.println(RTCTime.minutes);
	Debug_Serial.print("seconds: "); Debug_Serial.println(RTCTime.seconds);
	...
	Debug_Serial.println("Update RTC SUCCESS... <Update_RTC>");
}
...
void date::Set_Alarm(void)
{
	...
	alarm += Cyclic_interval;//设置闹钟时间
	Debug_Serial.println(String("设置的循环间隔时间Cyclic_interval = ") + Cyclic_interval);
...
void date::Set_onehour_Alarm(void)
{
	...
	alarm += 3600;
	Debug_Serial.println(String("电池电压低于2.8V!!!直接进入休眠，设置的间隔休眠时间为1小时"));
	Date.createAlarm(RTC_Interrupt, alarm);
	delay(10);
	bkp_disable_writes();
}
