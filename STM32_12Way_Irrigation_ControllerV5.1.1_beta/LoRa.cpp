...
unsigned char LoRa::Detect_Error_Receipt(unsigned char *verify_data)
{
	...
	if (verify_data[0] == 'E' && verify_data[1] == 'R')
	{
		...
		Debug_Serial.println(Error);  //打印出该错误代号
		return Error;
	}
	else
		return No_Err;
...
Receive_Type LoRa::AT_Query_Cmd(const char *cmd, unsigned char *data_buffer, unsigned char *data_len = 0)
{
	...
	if (ReceiveLen == 0)
		Debug_Serial.println("No receive data !!!");
...
{	
	...
	Debug_Serial.println(String("Str_CSQ = ") + Str_CSQ);
	Debug_Serial.print(String("Str_SNR = "));
    data_buffer[2] == 0xF0?Debug_Serial.print("-"):(data_buffer[2] == 0xE0?Debug_Serial.print(""):Debug_Serial.print("读取错误"));
	Debug_Serial.println(Str_SNR + " (DEC)");

    Debug_Serial.print(String("Str_RSSI = "));
    data_buffer[3] == 0xF0?Debug_Serial.print("-"):(data_buffer[3] == 0xE0?Debug_Serial.print(""):Debug_Serial.print("读取错误"));
    Debug_Serial.println(Str_RSSI + " (DEC)");
...
bool LoRa::LoRa_AT(unsigned char *data_buffer, bool is_query, const char *cmd, const char *para)
{
	...
	switch (ReturnType)
	{
	case ERROR: Debug_Serial.println("Receipt ERROR...");  Mode(PASS_THROUGH_MODE); return false; break;
	case Invalid: Debug_Serial.println("Involid!"); Mode(PASS_THROUGH_MODE); return false; break;

	case OK: Debug_Serial.println("Set para OK <LoRa_AT>"); break;
	case Bytes:for (unsigned char i = 0; i < ReceiveLength; i++)
				{
					Debug_Serial.print(data_buffer[i], HEX);
					Debug_Serial.print(" ");
				}
...
bool LoRa::Rewrite_ID(void)
{
	...
	{
		Debug_Serial.println("Read LoRa ADDR Err <Rewrite_ID>");
		return false;
	}
	...
	{
		Debug_Serial.println("Verify LoRa Addr <Rewrite_ID>");
		...
		if (LoRa_Para_Config.Read_LoRa_Addr((unsigned char *)EP_Buffer))
		{
			...
			if (!VerifyFlag)
			{
				Debug_Serial.println("LoRa addr for AT Error!, write EP addr to LoRa! <Rewrite_ID>");
			...
			{
				Debug_Serial.println("LoRa addr for AT Correct, OK <Rewrite_ID>");
				Debug_Serial.print("LoRa addr is:");
                    Debug_Serial.print(EP_Buffer[6]);
                    Debug_Serial.print(EP_Buffer[7]);
                    Debug_Serial.print(EP_Buffer[4]);
                    Debug_Serial.print(EP_Buffer[5]);
                    Debug_Serial.print(EP_Buffer[2]);
                    Debug_Serial.print(EP_Buffer[3]);
                    Debug_Serial.print(EP_Buffer[0]);
                    Debug_Serial.print(EP_Buffer[1]);
                Debug_Serial.println();
			}
		}
		...
		{
			Debug_Serial.println("EP saved err, rewrite! <Rewrite_ID>");
...
bool LoRa::Param_Check(const char *cmd, const char *para, bool only_set)
{
	...
	if (!only_set)
	{
		...
		{
			Debug_Serial.println("Query parameters Faild! <Param_Check>");
			Debug_Serial.println("查询参数失败！ <Param_Check>");
			return false;
		}
		...
		for (unsigned char i = 0; i < param_len; i++)
		{
			...
			{
				Debug_Serial.println("The parameters are different from what is expected. Reset the parameters <Param_Check>");
				Debug_Serial.println("这些参数与预期的不同。重新设置参数 <Param_Check>");
				QueryOKFlag = false;
				break;
			}
		}
		if (QueryOKFlag)
		{
			Debug_Serial.println("Read param correct...读取参数正确");
			return true;
		}
	}
	...
	if (QueryOKFlag == false || only_set == true)
	{
		...
		{
			Debug_Serial.println("Parameters set Err! 参数设置错误!<Param_Check>");
			return false;
		}
		return false;
	}
...
void LoRa::Parameter_Init(bool only_net)
{
	...
	Debug_Serial.println("Configurate LoRa parameters...配置LORA参数");
	...
	do {
		...
		if (!only_net)
		{
			...
			if (Lora_TRMode == 0x01)
			{
				Debug_Serial.println("Lora_TRMode == 0x01");
			...
			{
				Debug_Serial.println("Lora_TRMode == 0x02");
			...
			{
				Debug_Serial.println(String("不存在的Lora_TRMode：") + Lora_TRMode + ".配置为初始模式");
				StatusBuffer[i++] = Param_Check(AT_TFREQ_, "1C578DE0", false);//475.500.000
				StatusBuffer[i++] = Param_Check(AT_RFREQ_, "1C03A180", false);//470.000.000
			}
			...
			if (LORA_SYNC == 34)
			{
				Debug_Serial.println("LORA_SYNC == 0x34");
				StatusBuffer[i++] = Param_Check(AT_SYNC_, "34", false);
			}
			else if (LORA_SYNC == 12)
			{
				Debug_Serial.println("LORA_SYNC == 0x12");
				StatusBuffer[i++] = Param_Check(AT_SYNC_, "12", false);
			}
			else
			{
				Debug_Serial.println(String("不存在的LORA_SYNC：") + LORA_SYNC + ".配置为初始模式");
				StatusBuffer[i++] = Param_Check(AT_SYNC_, "34", false);
			}
	...
	Debug_Serial.println("LoRa parameters set ERROR! <Parameter_Init>");
	Debug_Serial.println("LORA参数配置错误! <Parameter_Init>");
	delay(3000);
	iwdg_feed();
