		...
		switch (which_Way-1)
		{
			case 0:
			{
				for (unsigned char i = 0; i < 5; i++){
					data[i] = analogRead(AI1);delay(2);
				}

				Data = Median_filtering(&data[0],5,false);break;
			}
			case 1:
			{
				for (unsigned char i = 0; i < 5; i++){
					data[i] = analogRead(AI2);delay(2);
				}
				Data = Median_filtering(&data[0],5,false);break;
			}
			case 2:
...
/*
 ...
 @return  :
 */
unsigned short Modbus_Coils::Median_filtering(unsigned short *filter_buf ,unsigned char FILTER_N ,bool Calculate_Average)
{
	unsigned short filter_temp = 0;
	unsigned int filter_sum = 0;
	unsigned short filter_data = 0;

	// 采样值从小到大排列（冒泡法）
	for(unsigned char j = 0; j < FILTER_N; j++) 
	{
		for(unsigned char i = 0; i < FILTER_N - j; i++) 
		{
	...
	{
		// 去除最大最小极值后求平均
		for(unsigned char i = 1; i < FILTER_N; i++) filter_sum += filter_buf[i];

		filter_data = (unsigned char)filter_sum / (FILTER_N - 2);
	}
	else
	{
		filter_data = filter_buf[(FILTER_N-1)/2];
	}
...
film_u32 Film_Read_Analog_Ele_Current_CH(film_u8 ch)//读取一路电机的电流值，单位是mA
{
	...
	if (Pos_Nega_mode.Read_EleCurrent_Times() >= 8)
		return data*Pos_Nega_mode.Read_EleCurrent_Times();
	else
		if (data >= 590)	return (data-590)*Pos_Nega_mode.Read_EleCurrent_Times();
		else	return 0;	
}
