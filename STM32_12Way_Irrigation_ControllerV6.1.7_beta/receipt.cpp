...
void Film_Status_Msg_Delivery_Task(film_u8 ch, film_m_sta sta)
{
	...
	// if (sta == Film_M_MEM_Exp)
	// {
	// 	unsigned char Unkonwn_Open_RunOpen = 0;
	// 	Unkonwn_Open_RunOpen = Film_Read_Run_Opening_CH(ch);//备份上一次开度为
	// 	Debug_Serial.println("");
	// 	Debug_Serial.println(String("第") + ch + "路开度未知，目标开度为：" + Unkonwn_Open_RunOpen);
	// 	Debug_Serial.println(String("第") + ch + "路开始重设开度...");

	// 	if (Unkonwn_Open_RunOpen >= 0 && Unkonwn_Open_RunOpen <= 100)
	// 	{
	// 		Film_Set_Open_Value(&ch,1,&Unkonwn_Open_RunOpen);
	// 		if (Film_New_Task_Handler(&ch,1,FILM_ROLL) == Film_OK)
	// 			Film_Motor_Run_Task(&ch,1,FILM_ROLL);
	// 	}
	// }
}
