...
bool Positive_Negative_MODE::Save_EleCurrent_Times(unsigned char data)
{
	...
	unsigned char EleCurrent_CRC = GetCrc8(&data,1);
	AT24CXX_WriteOneByte(EleCurrent_Times_CRC,EleCurrent_CRC);

	EEPROM_Write_Disable();

	if (AT24CXX_ReadOneByte(EleCurrent_Times) == data && AT24CXX_ReadOneByte(EleCurrent_Times_CRC) == EleCurrent_CRC)
	{
		Info_Println("保存电流倍数成功,倍数为: " + data + ",校验结果为: " + EleCurrent_CRC);
		return true;
	}
	else
	{
...
unsigned char Positive_Negative_MODE::Read_EleCurrent_Times(void)
{
	...
	for (unsigned char i = 0; i < 10; i++)
	{
		...
		data_CRC = GetCrc8(&data,1);
		if (data_CRC == AT24CXX_ReadOneByte(EleCurrent_Times_CRC))
		{
			return data;
		}
	}

	Info_Println("电流倍数存储异常,重新设置为默认值!!!");
	Pos_Nega_mode.Save_EleCurrent_Times(5);
