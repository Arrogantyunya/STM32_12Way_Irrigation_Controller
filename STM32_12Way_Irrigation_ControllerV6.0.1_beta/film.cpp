...
void Film_Check_Exp_Open_Task(void)
{
  ...
  for (i = 0; i < MOTOR_CHANNEL; i++)
  {
    if (Film_MEM_Read_Param(FILM_MEM_RUN_OPEN_BASE_ADDR, &film_pcb.run_open_val[0]) ||
      Film_MEM_Read_Param(FILM_MEM_RT_OPEN_BASE_ADDR, &film_pcb.rt_open_val[0]))
      continue;
...
film_err Film_New_Task_Handler(film_u8 *ch_buf, film_u8 ch_num, film_m_act act)
{
  ...
  for (i = 0; i < ch_num; i++)
  {
    ...
    {
      if (film_pcb.m_reset_channel & (1 << (ch_buf[i] - 1)))  continue;
      film_pcb.m_f_open_channel &= ~(1 << (ch_buf[i] - 1));
      film_pcb.m_open_channel &= ~(1 << (ch_buf[i] - 1));
    }
...
void Film_Set_Motor_Dir(film_m_act act)
{
  ...
  FILM_TRA_MOTOR(i, obj_channel)
    switch (act)
    {
      ...
      case FILM_ROLL : 
        if (film_pcb.rt_open_val[i] == FILM_UNKONWN_OPEN)
...
{
  Film_MEM_Set_Flag(FILM_MEM_ELE_CUR_FLAG_BASE_ADDR, FILM_MEM_FLAG_RESET_MODE);
}
