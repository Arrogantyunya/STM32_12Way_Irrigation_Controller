...
void Modbus::receivePDU(byte* frame) {
	...
	Serial.println(String("field1 = ") + String(field1, HEX));//0000,起始地址
	Serial.println(String("field2 = ") + String(field2, HEX));//0040,数据量
...
void Modbus::exceptionResponse(byte fcode, byte excode) {
	Serial.println("异常响应!!!<exceptionResponse>");
    //Clean frame buffer
    free(_frame);
    _len = 2;
