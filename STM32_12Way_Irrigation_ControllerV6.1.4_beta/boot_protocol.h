...
/**
 *The length of bytes taken by boot software version.
 **Function is a basis for applying the size of frame data cache of related protocol.
 */
#define BOOT_SOFTWARE_VER_LEN			3

/**
 *The length of bytes taken by run software version.
 *Function is a basis for applying the size of frame data cache of related protocol.
 */
#define RUN_SOFTWARE_VER_LEN				3
...
#define BT_DP_BUF_SIZE						(BT_DP_SIZE * 8)
...
#define BT_REPLY_DEV_VER_FRAME_MAIN_LEN			(BT_GENERAL_CONST_LEN + BT_ROLLBACK_FLAG_LEN + CHECK_VER_PERIOD_LEN \
																						  + BOOT_SOFTWARE_VER_LEN + RUN_SOFTWARE_VER_LEN + HARDWARE_VER_LEN + BT_CRC_LEN \
																						 + BT_FRAME_END_LEN)

/** 
 *Gets the length of the packet frame body (excluding the optional reserved bit and date)
 */
#define BT_ACQUIRE_GET_DATA_FRAME_MAIN_LEN		(BT_GENERAL_CONST_LEN + RUN_SOFTWARE_VER_LEN + BT_DATA_PACK_LEN \
...
enum BT_R_RESULT{
	...
	BT_R_DP_UD_OK	= 0x0C,	//Firmware package update completed
	BT_R_RE_CHECK_ERR = 0x0D //Reservation check is not valid
};
...
struct BT_pcb{
	...
	bt_u8 bt_sw_v_buf[2];	//Boot software version number. 
	
	bt_u8 sw_v_buf[2];	//Run software version number and hardware version number.
	...
	bt_u8 run_sta_process;				//BT process run status.
