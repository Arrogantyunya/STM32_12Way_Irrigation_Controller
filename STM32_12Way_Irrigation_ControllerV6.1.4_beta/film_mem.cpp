...
film_mem_err Film_MEM_Save_Param_CH(film_u32 w_base_addr, film_u8 *w_buf, film_u8 ch)
{
  ...
  if ((ch > MOTOR_CHANNEL) || !ch)
  {
    FM_PRINT((FM_PrintDBG_Buf, "The %d ERR [Film_MEM_Save_Param_CH]\n", ch));
    return FILM_MEM_CH_ERR;
  }
...
film_mem_err Film_MEM_Read_Param_CH(film_u32 r_base_addr, film_u8 *r_buf, film_u8 ch)
{
  ...
  if ((ch > MOTOR_CHANNEL) || !ch)
  {
    FM_PRINT((FM_PrintDBG_Buf, "The %d ERR [Film_MEM_Read_Param_CH]\n", ch));
    return FILM_MEM_CH_ERR;
  }
  ...
  do{
    if (try_num > FILM_MEM_READ_TRY_NUM) return FILM_MEM_R_ERR;

    if (Film_MEM_Read_Buffer(r_addr, &r_buf[0], t_size) || Film_MEM_Read_Buffer(crc_addr, &crc_tmp, 1))
    {
      try_num++;
      continue;
    }
    ...
    if (crc_val == crc_tmp) return FILM_MEM_OK;

    try_num++;
    FM_PRINT((FM_PrintDBG_Buf, "The %dth motor read single CRC ERR, try_num times = %d [Film_MEM_Read_Param_CH]\n", ch, try_num));
...
{
  if (Film_MEM_Read_Buffer(r_base_addr, &flag_buf[0], MOTOR_CHANNEL) != FILM_MEM_OK) return FILM_MEM_R_ERR;
  return FILM_MEM_OK;
}
...
film_mem_err Film_MEM_Set_Flag_CH(film_u32 w_base_addr, film_u8 ch, film_flag_mode mode)
{
  ...
  if ((ch > MOTOR_CHANNEL) || !ch)
  {
    FM_PRINT((FM_PrintDBG_Buf, "The %d ERR [Film_MEM_Get_Flag_CH]\n", ch));
    return FILM_MEM_CH_ERR;
  }
...
film_mem_err Film_MEM_Get_Flag_CH(film_u32 r_base_addr, film_u8 *flag_buf, film_u8 ch)
{
  ...
  do{
    ...
    Film_MEM_Read_Buffer(r_addr, &flag_buf[0], sizeof(film_u8));

    if (flag_buf[0] == FILM_MEM_RESET_FLAG || flag_buf[0] == FILM_MEM_SET_FLAG) return FILM_MEM_OK;

    try_num++;
    FM_PRINT((FM_PrintDBG_Buf, "The %dth motor read flag ERR, Try times = %d [Film_MEM_Get_Flag_CH]\n", ch, try_num));
  }while (1);
}
