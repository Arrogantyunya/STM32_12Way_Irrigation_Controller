...
#include "../bt_inc/boot_mem.h"
#include "../bt_inc/boot_crc16.h"
...
{
	BT_DEBUGF(BT_CB_DBG, (BT_PrintDBG_Buf, "[BT_Fill_ReservedByte] function is not implemented!\n"));
}

/**
 @brief		: When the server directive is received, the developer determines the method of checking the reserved field.
 
 @param		: Protocol type of server directive.
 
 @return	: Correct checks return 0 and error returns 1.
 */
__attribute__((weak)) bt_u8 BT_ReservedByte_Check(bt_u16 bt_type, bt_u8 *re_buf) 
{ 
	BT_DEBUGF(BT_CB_DBG, (BT_PrintDBG_Buf, "[BT_ReservedByte_Check] function is not implemented!\n"));
	return 0; 
}
...
{
	BT_DEBUGF(BT_CB_DBG, (BT_PrintDBG_Buf, "[BT_Write_DP_To_Flash] function is not implemented!\n"));
	return 1;
}

__attribute__((weak)) bt_u8 BT_Read_DP_From_Flash(bt_u32 write_addr, BT_DATAPACK_TYPE *app_buffer, bt_u32 app_len)
{
	BT_DEBUGF(BT_CB_DBG, (BT_PrintDBG_Buf, "[BT_Read_DP_From_Flash] function is not implemented!\n"));
	return 1;	
}
...
{
	BT_DEBUGF(BT_CB_DBG, (BT_PrintDBG_Buf, "[BT_APP_Check] function is not implemented!\n"));
	return 1;
}
...
{
	BT_DEBUGF(BT_CB_DBG, (BT_PrintDBG_Buf, "[BT_Stop_Interrupt] function is not implemented!\n"));
}

__attribute__((weak)) void BT_Start_Interrupt(void)
{
	BT_DEBUGF(BT_CB_DBG, (BT_PrintDBG_Buf, "[BT_Start_Interrupt] function is not implemented!\n"));
}
...
{
	BT_DEBUGF(BT_CB_DBG, (BT_PrintDBG_Buf, "[BT_Jump_APP_Handler] function is not implemented!\n"));
}

__attribute__((weak)) void BT_Jump_BOOT(bt_u32 boot_addr) 
{
	BT_DEBUGF(BT_CB_DBG, (BT_PrintDBG_Buf, "[BT_Jump_BOOT] function is not implemented!\n"));
}

__attribute__((weak)) bt_u8 BT_Exception_trigger(void) 
{
	BT_DEBUGF(BT_CB_DBG, (BT_PrintDBG_Buf, "[BT_Exception_trigger] function is not implemented!\n"));
	return 0;
}
...
bt_u8 BT_Get_RunStatus(void){return bt_pcb.run_sta;}
void BT_Set_RunStatus(bt_u8 run_sta){bt_pcb.run_sta = run_sta;}

void BT_Hex_To_Dec(bt_u8 *dat, bt_u8 len)
{
	bt_u8 temp1, temp2;

	for (bt_u8 i = 0; i < len; i++)
	{
		temp1 = dat[i] / 16;
		temp2 = dat[i] % 16;
		dat[i] = temp1 * 10 + temp2;
	}
}

void BT_Dec_To_Hex(bt_u8 *dat, bt_u8 len)
{
	bt_u8 temp1, temp2;
...
void BT_APP_Init(void)
{
	...
	{ 
		...
		{
			if (BT_APP_Check(BT_DP_SAVE_BASE_ADDR))	//If the program storage area check success.
			{
				BT_Set_RunStatus(CP_DP);
				BT_Read_SaveArea_DP_Size(&dp_size);
				BT_DEBUGF(BT_APP_DBG, (BT_PrintDBG_Buf, "(Run APP excp) New Program storage area size : 0x%X\n", dp_size));
	...
	else
	{
		if (BT_Read_DP_Copy_Flag())	//If there is a copy request flag(update app)
		{
			if (BT_APP_Check(BT_DP_SAVE_BASE_ADDR))	//If the program storage area check success.
			{
				BT_Set_RunStatus(CP_DP);
				BT_Read_SaveArea_DP_Size(&dp_size);
				BT_DEBUGF(BT_APP_DBG, (BT_PrintDBG_Buf, "(Run APP OK) New Program storage area size : 0x%X\n", dp_size));
				BT_Copy_DataPack(dp_size);
				BT_Clear_DP_Copy_Flag();//Clear the copy request flag.
...
void BT_Mount_Dev_And_Init(bt_s_h_func s_h_func)
{
	...
	if (BT_Exception_trigger())	BT_Set_RunStatus(APP_EXP);
}
...
void BT_Version_Handle(bt_u8 *v_buf, bt_u8 is_rollback) 
{
	...
	BT_Hex_To_Dec(&v_buf[0], 2);
	rcv_s_v_value = ((v_buf[0] << 8) | v_buf[1]);
	BT_DEBUGF(BT_V_DBG, (BT_PrintDBG_Buf, "Software version number received : %X-%X\n", v_buf[0], v_buf[1]));  
	...
	BT_DEBUGF(BT_V_DBG, (BT_PrintDBG_Buf, "Running software version : %X-%X\n", read_s_v_value >> 8, read_s_v_value & 0xFF));

	if ((rcv_s_v_value <= read_s_v_value) && (!is_rollback)) 
	{
		BT_DEBUGF(BT_V_DBG, (BT_PrintDBG_Buf, "No new firmware\n"));
		return;
	}
 ...
 */
static void 
BT_Fill_Frame_Content(bt_u16 *pframe_index, bt_u16 pframe_data_len, bt_u16 frame_type)
{
	...
	*pframe_index = 0;
	...
	bt_pcb.psend_frame[(*pframe_index)++] = BT_FRAME_HEAD;
	bt_pcb.psend_frame[(*pframe_index)++] = BT_HighByte(pframe_data_len);
	bt_pcb.psend_frame[(*pframe_index)++] = BT_LowByte(pframe_data_len);
	/* The frame serial number */
	for (bt_u8 i = 0; i < 5; i++)
		bt_pcb.psend_frame[(*pframe_index)++] = bt_pcb.f_index_buf[i];
	bt_pcb.psend_frame[(*pframe_index)++] = BT_HighByte(frame_type);
	bt_pcb.psend_frame[(*pframe_index)++] = BT_LowByte(frame_type);
	bt_pcb.psend_frame[(*pframe_index)++] = BT_HighByte(BT_DEVICE_ID);
	bt_pcb.psend_frame[(*pframe_index)++] = BT_LowByte(BT_DEVICE_ID);
	bt_pcb.psend_frame[(*pframe_index)++] = BT_M_S_DEV_FLAG;
	
	switch (frame_type)
	{ 
		case BT_REPLY_DEV_VER_FRAME :
			/* Remote rollback of the flag */
			bt_pcb.psend_frame[(*pframe_index)++] = BT_ROLLBACK_FLAG;
			/* Query cycle value */
			BT_Read_Query_Cycle_Value(&bt_pcb.que_v_cyc_val);
			bt_pcb.psend_frame[(*pframe_index)++] = BT_HighByte(bt_pcb.que_v_cyc_val);	
			bt_pcb.psend_frame[(*pframe_index)++] = BT_LowByte(bt_pcb.que_v_cyc_val);
			/* Software version and hardware version */
			BT_Read_S_Version(&bt_pcb.sw_hw_v_buf[0]);
			BT_Dec_To_Hex(&bt_pcb.sw_hw_v_buf[0], 4);
			for (i = 0; i < 4; i++)
			{
				bt_pcb.psend_frame[(*pframe_index)++] = bt_pcb.sw_hw_v_buf[i];
				if (i % 2 != 0)
					bt_pcb.psend_frame[(*pframe_index)++] = 0xE1;
			}
		break;
		
		case BT_ACQUIRE_DATA_FRAME :
			BT_Read_Temp_S_Version(&bt_pcb.sw_hw_v_buf[0]);
			BT_Dec_To_Hex(&bt_pcb.sw_hw_v_buf[0], 4);
			bt_pcb.psend_frame[(*pframe_index)++] = bt_pcb.sw_hw_v_buf[0];
			bt_pcb.psend_frame[(*pframe_index)++] = bt_pcb.sw_hw_v_buf[1];
			bt_pcb.psend_frame[(*pframe_index)++] = 0xE1;
			/* Firmware package length */
			bt_pcb.psend_frame[(*pframe_index)++] = BT_HighByte(BT_DP_SIZE);
			bt_pcb.psend_frame[(*pframe_index)++] = BT_LowByte(BT_DP_SIZE);
			/* Firmware package index */
			bt_pcb.psend_frame[(*pframe_index)++] = BT_HighByte(bt_pcb.d_pack_index);
			bt_pcb.psend_frame[(*pframe_index)++] = BT_LowByte(bt_pcb.d_pack_index);
		break;
		
		case BT_GENERAL_REPLY_FRAME :
			/* Receipt result */
			bt_pcb.psend_frame[(*pframe_index)++] = bt_pcb.r_result;	
		break;
	}
	
	/* Use length for reserved bits */
	bt_pcb.psend_frame[(*pframe_index)++] = RESERVED_LEN;
	/* Reserve bit data cache */
	for (i = 0; i < RESERVED_LEN; i++)
		bt_pcb.psend_frame[(*pframe_index)++] = bt_pcb.re_buf[i];
		
	bt_pcb.psend_frame[(*pframe_index)++] = BT_DATE_USE_FLAG;
	...
	bt_pcb.psend_frame[(*pframe_index)++] = BT_HighByte(frame_crc_val);
	bt_pcb.psend_frame[(*pframe_index)++] = BT_LowByte(frame_crc_val);
	
	/* Frame tail */
	for (i = 0; i < BT_FRAME_END_LEN; i++)
		bt_pcb.psend_frame[(*pframe_index)++] = (i % 2 == 0) ? 0x0D : 0x0A;
}
...
/**
 @brief		: This function is provided by the protocol file, 
						periodically query the latest software version of the device.
...
void BT_Cycle_Query_SW_Version(void)
{
	...
	if (bt_pcb.d_pack_en_get_flag)
		BT_Set_RunStatus(CYC_QUE_DP);
	else
		BT_Set_RunStatus(CYC_QUE_VER);
...
#endif
{
	...
	if(!BT_Get_Rcv_Reserved(frame_type)) return BT_CHECK_ERR;
	...
	switch (frame_type)
	{
		...
		case BT_REPLY_DEV_VER_FRAME :
			...
			{
				if ((i % 2 == 0) && i) j++;
				bt_pcb.sw_hw_v_buf[i] = bt_pcb.prcv_frame[16 + j++];
			}
			BT_Save_Query_Cycle_Value((bt_pcb.prcv_frame[14] << 8 | bt_pcb.prcv_frame[15]));
			BT_Version_Handle(&bt_pcb.sw_hw_v_buf[0], bt_pcb.prcv_frame[13]); 
		break;
		...
		case BT_FORCE_UPDATA_FRAME : 
			...
			for (i = 0; i < 2; i++)
				bt_pcb.sw_hw_v_buf[i] = bt_pcb.prcv_frame[14 + i];
			BT_Version_Handle(&bt_pcb.sw_hw_v_buf[0], bt_pcb.prcv_frame[13]);
		break;
...
{ 
	...
	if (dp_len != BT_DP_SIZE && !bt_pcb.d_pack_end_flag) 
	{
