...
bool ModbusSerial::receive(byte* frame) {
    ...
    if (address != 0xFF && address != this->getSlaveId()) {
		Debug_Serial.println("Modbus address error!!! <ModbusSerial::receive>");
		return false;
	}

    //CRC Check
    if (crc != this->calcCrc(_frame[0], _frame+1, _len-3)) {
		Debug_Serial.println("Modbus crc error!!! <ModbusSerial::receive>");
		return false;
    }
...
bool ModbusSerial::send(byte* frame) {
	...
	for (i = 0; i < _len; i++)
	{
		...
		Debug_Serial.print(String(G_modbusPacket[i], HEX) + " ");
	}
	Debug_Serial.println("ModbusSerial::send <ModbusSerial::send>");
...
bool ModbusSerial::sendPDU(byte* pduframe) {
	...
	Debug_Serial.println("ModbusSerial::sendPDU <sendPDU>");
	//G_modbusPacket[G_modbusPacket_Length++] = *(frame + i);
...
void ModbusSerial::task(unsigned char* modbusPacket ,int Length) {
	...
	Debug_Serial.println("Test <task>");
	byte i;
	_frame = (byte*)malloc(_len);
	//for (i = 0; i < _len; i++) _frame[i] = (*_port).read();
	...
	{
		Debug_Serial.println(_frame[i], HEX);
	}
	Debug_Serial.println("--------");
